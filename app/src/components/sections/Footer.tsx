...
export default function Footer() {
  return (
    <div className="bg-gray-700 bg-icons-gray flex flex-col md:flex-row gap-4 justify-between items-center px-5 sm:px-36 pb-5 sm:py-5 w-full mx-auto">
        ...
        <div className="flex space-x-10">
          {icons.map((icon) => (
            <a
              href={icon.link}
              key={icon.name}
              className="w-12 h-12 bg-gray-700 border-2 border-solid border-l-white rounded-full flex items-center justify-center hover:filter hover:invert">
              <Image
                src={icon.icon}
                alt={icon.name}
                height={48}
